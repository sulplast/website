...
class ComercialController extends Controller {
	...
	public function index(Request $request)
	{
		if (request()->isMethod('post')) {
			...
			try {
				Mail::send('frontend.emails.comercial', ['input' => $request->all()], function($message) use ($request) {
					$message->to('sulplast@sulplast.com.br')
						->subject('[sulplast.com.br] - Comercial');
		...
		return view('frontend.comercial.index');
	}
